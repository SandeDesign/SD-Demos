  ...
  return (
    <div className="min-h-screen bg-[#0a0b1a] text-white overflow-x-hidden">
      ...
      <SiteHeader
        navItems={[
          { label: 'Home', href: '/' },
          { label: 'Onepage', href: '/standaard' },
          { label: 'Voorbeelden', href: '#voorbeelden' },
        ]}
        ctaLabel="Start aanvraag"
        ctaHref="https://www.sandedesign.nl/website-aanvraag/uitgebreid"
      />
              ...
              <div className="lg:hidden mb-8">
                <div className="bg-[#1a1b2e] rounded-xl border border-white/10 shadow-xl overflow-hidden cursor-pointer" onClick={() => setFullPreview(iframeSrc)}>
                  <div className="flex items-center gap-2 px-3 py-2 border-b border-white/5">
                    ...
                    <div className="flex-1 mx-2 px-2 py-0.5 rounded bg-white/5 text-[10px] text-gray-500 text-center">jouwplatform.nl</div>
                ...
                <div className="flex items-center justify-center gap-2 mt-3">
                  {previewPaths.map((_, i) => (
                    <button key={i} onClick={() => setCurrentPreview(i)} className={`w-1.5 h-1.5 rounded-full transition-all ${i === currentPreview ? 'w-4 bg-pink-400' : 'bg-white/20'}`} />
                  ))}
                </div>
              </div>
